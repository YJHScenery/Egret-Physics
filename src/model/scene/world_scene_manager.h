/**
 * @file        world_scene_manager.h
 * @brief       世界场景管理器头文件，定义物理世界的场景管理功能。
 * @details     定义 WorldSceneManager 类及其相关结构，负责管理物理世界中的所有实体、
 *              包括刚体、粒子、场和约束等。
 *
 * @author      作者姓名 <作者邮箱>
 * @date        2026-05-04
...
namespace egret
{
     ...
     * @details     BodyRecord 是世界中的实体记录结构体，保存实体所有权与可渲染/可求解数据。
     *              包含实体ID、名称、物理实体所有权、碰撞标志、积分标志等。
     *              用于场景管理器管理物理实体。
     *
     * @invariant   id 是唯一的实体标识符
     * @invariant   entity 是有效的物理实体共享指针
     * @see         WorldSceneManager, PhysicalEntity
     */
    struct BodyRecord
    {
        /** 实体 ID。 */
     ...
     * @details     FieldRecord 是世界中的场记录结构体，保存场实例与名称。
     *              包含场ID、名称、场的所有权等。
     *              用于场景管理器管理物理场。
     *
     * @invariant   id 是唯一的场标识符
     * @invariant   field 是有效的场共享指针
     * @see         WorldSceneManager, FieldBase
     */
    struct FieldRecord
    {
        /** 场 ID。 */
     ...
     * @details     ConstraintRecord 是世界中的约束记录结构体，保存约束实例与名称。
     *              包含约束ID、名称、约束的所有权、启用标志等。
     *              用于场景管理器管理约束器。
     *
     * @invariant   id 是唯一的约束标识符
     * @invariant   constraint 是有效的约束共享指针
     * @see         WorldSceneManager, ConstraintsBase
     */
    ...
    /**
     * @brief       场景记录，保存完整的场景数据。
     * @details     SceneRecord 是场景记录结构体，保存完整的场景数据。
     *              包含实体列表、场列表、约束列表等。
     *              用于场景的序列化和反序列化。
     *
     * @invariant   bodies 包含所有实体记录
     * @invariant   fields 包含所有场记录
     * @invariant   constraints 包含所有约束记录
     * @see         WorldSceneManager, BodyRecord, FieldRecord, ConstraintRecord
     */
    struct SceneRecord
    {
     ...
     * @details     SceneRenderItem 是用于视图层渲染的轻量化数据快照结构体。
     ...
     * @see         WorldSceneManager, SceneBodyModel
     */
     ...
     * @details     WorldSceneManager 继承自 SolverSceneSnapshotBase，
     *              是模型层场景管理器，负责世界真相数据、时间、ID 和求解调度。
     *              职责说明：
     *              1. 管理实体与场的创建、删除、遍历和查询。
     *              2. 维护仿真时间与步数。
     *              3. 作为求解器的场景快照适配器，向求解器提供只读/可写句柄。
     *              4. 为 ViewModel 层提供可渲染的轻量化数据快照。
     *              采用组合模式，通过 Solver 推进物理模拟。
     *
     * @invariant   m_solver 是有效的求解器实例
     * @invariant   m_bodies 包含所有实体记录
     * @invariant   m_fields 包含所有场记录
     * @invariant   m_constraints 包含所有约束记录
     * @remark      WorldSceneManager 不是线程安全的，多线程访问需要外部同步
     * @see         SolverSceneSnapshotBase, Solver, BodyRecord, FieldRecord, ConstraintRecord
     */
